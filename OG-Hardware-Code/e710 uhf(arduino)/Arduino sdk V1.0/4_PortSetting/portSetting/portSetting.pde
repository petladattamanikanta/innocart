...
void setup() {
  ...
  byte port[64], num = 0, result, i;

  //test Set Port setting
  port[num++] = 1;              // set port 1,2,3,4
  port[num++] = 2; 
  port[num++] = 3; 
  port[num++] = 4; 
  Serial.println("SetPortSetting!");
  result = SetPortSetting(port, num, 1);                // saveflag = 1
  if (result == RT_TRUE)
  {
  ...
  num = 0;
  // test Get Port setting
  Serial.println("GetPortSetting!");
  result = GetPortSetting(port, &num);
  if (result == RT_TRUE)
  {
    Serial.println("Get OK!");    
    Serial.println(num, DEC);
    for(i=0; i<num; i++)
      Serial.println(port[i], DEC);
  }
  else
  {
...
void loop() {
  ...
  Serial.println("Port Set and Get test!");
  delay(1000);
}
