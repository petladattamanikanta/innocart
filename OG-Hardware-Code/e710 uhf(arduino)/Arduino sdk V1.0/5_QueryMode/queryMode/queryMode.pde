...
void setup() {
  ...
  byte mode = 0xFF, result;

  //test Set query mode
  Serial.println("SetQueryMode!");
  mode = 0;      // Multi tag mode
  result = SetQueryMode(mode, 1);                // saveflag = 1
  if (result == RT_TRUE)
  {
  ...
  // test Get query mode
  Serial.println("GetQueryMode!");
  result = GetQueryMode(&mode);
  if (result == RT_TRUE)
  {
...
void loop() {
  ...
  Serial.println("Query Mode Set and Get test!");
  delay(1000);
}
