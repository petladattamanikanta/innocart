...
void setup() {
  ...
  Serial.println("Test tag memory access!");
  Serial.println("press 'R' Read memory");
  Serial.println("press 'W' Write memory");
  Serial.println("press 'L' Lock memory");
  Serial.println("press 'U' Unlock memory");
  Serial.println("press 'K' Kill tag");
}
...
void loop() {
  ...
  if ( Serial.available())
  {
      ...
      if (cmd == 'R')
      {
          byte accpwd[4], mmb, mb, mdata[100], readdata[100];
          int  msa, mdl, sa, dl;
          char tmp[100];
        
          accpwd[0] = accpwd[1] = accpwd[2] = accpwd[3] = 0x00;          // access password = 0x00000000
          mmb = 0;
          msa = 0;
          mdl = 0;                      // mdl = 0, means no select
          mb = 1;                       // 0--RFU; 1--EPC; 2--TID;3--USR 
          sa = 2;                       // start addr
          dl = 6;                       // word
          result = ReadTagMemory(accpwd, mmb, msa, mdl, mdata, mb, sa, dl, readdata);
          if (result == RT_TRUE)
          {
              Serial.println("Read Tag Memory success!");    
              ByteToHexStr(readdata, tmp, dl*2);
              tmp[dl*4] = '\0';
              Serial.println(tmp);                   
          }
      }
      ...
      {
          byte accpwd[4], mmb, mb, mdata[100], writedata[100], flag;
          int  msa, mdl, sa, dl, timeout, i;
          char tmp[100];
          ...
          for(i=0; i<dl*2; i++)
              writedata[i] = i;
          flag = 0;                     // 0--normal write, 1-- if tag is more than 1 tag, not write,
          timeout = 0;                  // if flag = 0, timeout is not useful
          result = WriteTagMemory(accpwd, mmb, msa, mdl, mdata, mb, sa, dl, writedata, flag, timeout);
          if (result == RT_TRUE)
          {
              Serial.println("Write Tag Memory success!");                   
          }
      }
      else if (cmd == 'L')
      {
          ...
          accpwd[0] = accpwd[1] = accpwd[2] = accpwd[3] = 0x11;          // access password = 0x11111111
          mmb = 0;
          msa = 0;
          mdl = 0;                      // mdl = 0, means no select
          mb = 1;                       // 0--kill; 1--Acc; 2--EPC; 3--TID; 4--USR 
          flag = 1;                     // 0--unlock, 1--lock,
          result = LockTagMemory(accpwd, mmb, msa, mdl, mdata, mb, flag);
          if (result == RT_TRUE)
          {
              Serial.println("Lock Memory success!");                   
          }
      }
      else if (cmd == 'U')
      {
          ...
          mdl = 0;                      // mdl = 0, means no select
          mb = 1;                       // 0--kill; 1--Acc; 2--EPC; 3--TID; 4--USR 
          flag = 0;                     // 0--unlock, 1--lock,
          result = LockTagMemory(accpwd, mmb, msa, mdl, mdata, mb, flag);
          if (result == RT_TRUE)
          {
              Serial.println("UnLock Memory success!");                   
          }
      }
  }  
}
