...
void setup() {
  ...
  byte NumOfPort, result, i;
  float rPow[100], wPow[100];

  Serial.println("SetRFIDTxPower!");
  result = SetRFIDTxPower(1, 18.5, 9.3, 1);      // set ant 1, read power = 18.5dBm, write power = 9.3dBm, power off save 
  if (result == RT_TRUE)
  {
  ...
  Serial.println("GetRFIDTxPower!");
  result = GetRFIDTxPower(&NumOfPort, rPow, wPow);
  if (result == RT_TRUE)
  {
    Serial.println("Get OK!");
    Serial.println(NumOfPort, DEC);
    for (i=0; i<NumOfPort; i++)
    {
      Serial.println(i+1, DEC);
      Serial.println(rPow[i], 1);
      Serial.println(wPow[i], 1);
    }
  }
  else
  {
...
void loop() {
  ...
  Serial.println("Tx Power Set and Get test!");
  delay(1000);
}
