...
byte GetModuleTemperature(float *value)
{
  byte tmp[100], sendbuf[100], ret = RT_FALSE, cmd;
  int sendlen = 0, cur = 0;
  long timeout = 0x80000;      // time out (2300ms for Mega 2560)
  ...
  cmd = 0x34;
  Build_RFID_Frame(cmd, tmp, cur, sendbuf, &sendlen);        // cmd = 0x34
  serialSendFrameToRFID(sendbuf, sendlen);
  ...
  while(timeout--)
  {
    ...
    if (RT_TRUE == Rfid_Handle_Command())
    {
      if (stHostifRecvFrame.u8Cmd == cmd+1)
      {
        if (stHostifRecvFrame.pData[0] == 0x01)
        {
          *value = ((stHostifRecvFrame.pData[1]*256) + stHostifRecvFrame.pData[2]) / 100.0;
          ret = RT_TRUE; 
        }
...
void setup() {
  ...
  Serial.println("Test Get Module !");
  Serial.println("press 'G' Get Module Temperature");
}
...
void loop() {
  ...
  if ( Serial.available())
  {
      ...
      if (cmd == 'G')
      {
          ...
          result = GetModuleTemperature(&temperature);      
          if (result == RT_TRUE)
          {
              Serial.println("Get Module Temperature success!");   
              Serial.println(temperature); 
          }
          else
          {
              Serial.println("Get Module Temperature failed!");    
          }
      }      
  }  
}
